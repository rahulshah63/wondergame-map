...
const tick = () => {
  ...
  if (intersection) {
    ...
    ;(overlayHex[0].material as THREE.MeshBasicMaterial).opacity =
    ...
    if (Settings.showNeighbour) {
      ...
      for (let i = 0; i < 6; ++i) {
        if (hexgrid.isInRange(neighbours[i])) {
          ...
          ;(<THREE.MeshBasicMaterial>(<unknown>overlayHex[i + 1])).opacity = (<
            THREE.MeshBasicMaterial
          >overlayHex[0].material).opacity
          ;(<THREE.MeshBasicMaterial>(
            (<unknown>overlayHex[i + 1].material)
