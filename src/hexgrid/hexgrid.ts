...
class HexGrid {
  ...
  convertOffsetToWorldPosition(p: THREE.Vector3) {
    const px =
      this.topLeft.x +
      (p.x + p.z * 0.5 - Math.floor(p.z / 2)) * this.innerRadius * 2.0
  ...
  convertCubeToWorldCoordinate(p: THREE.Vector3): THREE.Vector3 {
    return this.convertOffsetToWorldPosition(
      this.convertCubeToOffsetCoordinate(p)
    )
  }
  ...
  convertToCubeCoordinate(position: THREE.Vector3) {
    ...
    if (!this.isInRange(new THREE.Vector3(ix, iy, iz))) return null
